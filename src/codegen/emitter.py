...
class Emitter:
    ...
    def emit_invoke_virtual(self, lexeme: str, in_, frame) -> str:
        ...
        if typ:
            list(map(lambda x: frame.pop(), typ.param_types))
            type_ = '(' + ''.join(list(map(lambda x: self.get_jvm_type(x), typ.param_types))) + ')' + self.get_jvm_type(typ.return_type)
        else:
            type_ = ""
        ...
        if typ:
            if not type(typ.return_type) is VoidType:
                frame.push()
        return self.jvm.emitINVOKEVIRTUAL(lexeme, type_)
    ...
    def emit_ifeq(self, label: int, frame) -> str:
        """
        Emit IFEQ instruction.
    ...
    def emit_new_array(self, typ: Type) -> str:
        """
        ...
        Returns:
            Generated JVM instruction string
        """
        return self.jvm.emitNEWARRAY(self.get_full_type(typ))

    def emit_anew_array(self, typ: Type) -> str:
            """
            Emit NEWARRAY instruction.
            ...
            if type(typ) is ArrayType:
                lexeme = self.get_jvm_type(typ)
            else:
                lexeme = self.get_full_type(typ)
            return self.jvm.emitANEWARRAY(lexeme)

    def emit_multi_anew_array(self, typ: Type, dimensions: int) -> str:
            """
            Emit MULTIANEWARRAY instruction.

            Args:
                typ: Array type string
                dimensions: Array dim
            Returns:
                Generated JVM instruction string
            """
            return self.jvm.emitMULTIANEWARRAY(self.get_full_type(typ), dimensions)
